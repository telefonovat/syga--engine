...
RGBA_REGEX_STR = r"rgba?\(\s*(?:(?:{i}{s}{i}{s}{i})|(?:{p}{s}{p}{s}{p}))(?:{s}(?:{f}|{p}))?\s*\)".format(
...
class Color:
    ...
    def is_rgba(color):
        ...
        if isinstance(color, (list, tuple)):
            ...
            if len(color) == 4 and (
                color[3] < -FLOAT_ERROR or color[3] > 1 + FLOAT_ERROR
            ):
                return False
    ...
    def is_color(color):
        ...
        return (
            ...
            or color is None
            or color == "default"
            or Color.is_keyword(color)
            or Color.is_hex(color)
            or Color.is_rgba(color)
        )
    ...
    def normalize_rgba(color):
        ...
        if isinstance(color, str):
            parts = [
                p.strip()
                for p in color.strip()[(4 + int("rgba" in color)) : -1].split(",")
            ]
            rgba = parts[:3]
            ...
            if len(parts) == 4:
                rgba.append(
                    parse_perc(parts[3]) if "%" in parts[3] else float(parts[3])
                )
