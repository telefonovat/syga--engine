...
class GraphShaper:
    ...
    def _binary_interpretation(self, true=None, false=None):
        ...
        """
        true_shape = (
            self.Shape.create(true) if true is not None else self.DEFAULT_TRUE_SHAPE
        )
        false_shape = (
            self.Shape.create(false) if false is not None else self.DEFAULT_FALSE_SHAPE
        )
    ...
    def _group_interpretation(self, shapes):
        ...
        if isinstance(shapes, int):
            ...
            if shapes > max_shapes:
                raise GraphShaperException(
                    f"Too many groups: {shapes}. There are only {max_shapes} shapes"
                )

            # Consider edges
            self._shapes = [
                self.Shape.create(shape) for shape in self.available_shapes[:shapes]
            ]
        ...
        if isinstance(shapes, list):  # todo: turn this into elif
            ...
            self._shapes = {
                key: self.Shape.create(shape) for key, shape in shapes.items()
            }
        ...
        if isinstance(self._shapes, list):
            ...
            if len(self._shapes) < len(uniq):
                if len(self._shapes) == 2:
                    ...
                    raise GraphShaperException(
                        f"Too few shapes: found {len(uniq)} unique values"
                    )
            else:
    ...
    def __init__(self, transform, **kwargs):
        ...
        if "shape" in kwargs and "shapes" in kwargs:
            raise GraphShaperException(
                "Shape and shapes arguments are mutually exclusive"
            )
        ...
        if not self._validate_shapes(self._shapes):
            raise GraphShaperException(
                f"Invalid value for parameter shape(s): {self._shapes}"
            )
...
class GraphNodeShaper(GraphShaper):
    ...
    def build(*args, **kwargs):
        ...
        if len(args) == 1:
            ...
            raise GraphNodeShaperException(
...
class GraphEdgeShaper(GraphShaper):
    ...
    def build(*args, **kwargs):
        ...
        if len(args) == 1:
            if isinstance(args[0], Iterable):
                return GraphEdgeShaper.build(
                    lambda u, v, G: (u, v) in args[0], **kwargs
                )

            if isinstance(args[0], types.FunctionType):
                return GraphEdgeShaper(args[0], **kwargs)

            raise GraphEdgeShaperException(
                f"Cannot use object of type {type(args[0])} as source"
            )
        ...
        transform = lambda u, v, G: (
            None if prop not in G.edges[u, v] else G.edges[u, v][prop]
        )
