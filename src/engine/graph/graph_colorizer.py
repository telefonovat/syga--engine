...
class GraphColorizer:
        ...
        if isinstance(self._colors, list):
            ...
            if len(self._colors) < len(uniq):
                if len(self._colors) == 2:
                    ...
                    raise GraphColorizerException(
                        f"Too few colors: found {len(uniq)} unique values"
                    )
            else:
    ...
    def __init__(self, transform, **kwargs):
        ...
        if "color" in kwargs and "colors" in kwargs:
            raise GraphColorizerException(
                "Color and colors arguments are mutually exclusive"
            )
        ...
        if self._palette is not None and self._colors is not None:
            raise GraphColorizerException(
                "Parameters color(s) and palette are mutually exclusive"
            )

        if not GraphColorizer.validate_colors(self._colors):
            raise GraphColorizerException(
                f"Invalid value for parameter color(s): {self._colors}"
            )

        if not GraphColorizer.validate_palette(self._palette):
            raise GraphColorizerException(
                f"Invalid value for parameter palette: {self._palette}"
            )

        if not GraphColorizer.validate_range(self._range):
            raise GraphColorizerException(
                f"Invalid value for parameter range: {self._range}"
            )
...
class GraphNodeColorizer(GraphColorizer):
    ...
    def build(*args, **kwargs):
        ...
        if len(args) == 1:
            ...
            raise GraphNodeColorizerException(
...
class GraphEdgeColorizer(GraphColorizer):
    ...
    def build(*args, **kwargs):
        ...
        if len(args) == 1:
            ...
            raise GraphEdgeColorizerException(
                f"Cannot use object of type {type(args[0])} as source"
            )
