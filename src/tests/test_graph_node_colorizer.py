...
class TestGraphNodeColorizer(unittest.TestCase):
    ...
    def _random_colors_argument(self):
        ...
        if opt == OPT_DICT:
            return {
                random_name(4): random_color() for _ in range(random.randint(1, 20))
            }
    ...
    def test_validate_color_arg_valid(self):
        ...
        for _ in range(200):
            ...
            self.assertTrue(
                GraphNodeColorizer.validate_colors(colors), f"colors={colors} is valid"
            )
    ...
    def test_validate_range_arg(self):
        ...
        for _ in range(200):
            ...
            self.assertEqual(
                ...
                valid,
                "Range {} is {}".format(
                    (lower, upper), "valid" if valid else "invalid"
                ),
            )
    ...
    def test_set_transformation(self):
        ...
        self.assertEqual(
            transformed, nodes, "Set transformation from the specified set"
        )
    ...
    def test_lambda_transformation(self):
        ...
        self.assertEqual(
            colorizer.transform(G), expected, "Lambda transformation using node ids"
        )
    ...
    def test_group_interpretation_two_colors_more_values(self):
        ...
        for prop, v in itertools.product(props, values):
            v = (
                2
                if isinstance(v, int)
                else list(v.values())[:2] if isinstance(v, dict) else v[:2]
            )
    ...
    def test_compute_single_group_interpretation_specified(self):
        ...
        palette = sns.color_palette(
            GraphNodeColorizer.DEFAULT_DISCRETE_PALETTE, len(colors)
        )
