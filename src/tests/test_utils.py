...
from src.utils.path import path_from_root, resolve_url
from src.utils.code import detect_indentation, add_indentation, get_sample_code
...
class TestUtils(unittest.TestCase):
    ...
    def test_detect_indentation(self):
        ...
        for indentation in random.choices(range(1, 11), k=10):
            ...
            self.assertEqual(
                detect_indentation(code), indentation, "Correctly detect indentation"
            )
    ...
    def test_add_indentation(self):
        ...
        for indentation in random.choices(range(1, 11), k=10):
            ...
            indentations_before = [
                len(line) - len(line.lstrip()) for line in code_before.splitlines()
            ]

            code_after = add_indentation(code_before, indentation)
            indentations_after = [
                len(line) - len(line.lstrip()) for line in code_after.splitlines()
            ]

            diff = [
                after - before
                for after, before in zip(indentations_after, indentations_before)
            ]
